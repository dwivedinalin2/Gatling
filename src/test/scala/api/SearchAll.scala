...
class SearchAll extends Simulation{

  //protocol
  val httpProtocol= http.baseUrl("https://api.landau.app/")

  //scenario
  val scn1=scenario("Search keyword")
    .exec(
      http("Search keyword")
        .post("/v1/search-all?limit=8&page=1 ")
        ...
        .header("content-type","application/json").asJson
        .body(RawFileBody("src/test/resources/searchkeyword.json")).asJson
        .check(
          status is 200
